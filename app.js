...
function createBasemapLayers(dark) {
  if (dark) {
    return {
      dark: true,
      primary: L.tileLayer(
  ...
  return { dark: false, primary, fallback };
}

function addBasemap(map) {
  const layers = createBasemapLayers(prefersDarkMap());
  if (layers.fallback) {
    let failed = 0;
    let switched = false;
    layers.primary.on("tileerror", function () {
      failed += 1;
      if (switched || failed < 4) return;
      switched = true;
      map.removeLayer(layers.primary);
      layers.fallback.addTo(map);
    });
  }
  layers.primary.addTo(map);
  map._qdBasemap = layers;
}
...
function refreshBasemapTheme() {
  ...
  ["main", "mini"].forEach(function (key) {
    ...
    if (prev.fallback && map.hasLayer(prev.fallback)) map.removeLayer(prev.fallback);
    addBasemap(map);
  });
}
...
function setNav(view) {
  ...
  const active = nav && nav.querySelector("a.is-active");
  if (nav && active) {
...
function viewCime() {
  const query = normalizeSearch(state.cimeSearch).trim();
  let lista = state.gruppoFilter
    ? state.cime.filter((c) => c.gruppo === state.gruppoFilter)
    : state.cime.slice();
  if (query) {
    lista = lista.filter((c) => normalizeSearch(c.nome).includes(query));
  }
  const rows = [...lista]
    .sort((a, b) => peakIdNum(a.id) - peakIdNum(b.id) || b.altezza_m - a.altezza_m)
    .map((c) => {
      const n = c.ascese.length;
    ...
    .join("");
  const scope = state.gruppoFilter
    ? " in " + escapeHtml(state.gruppoFilter)
  ...
  appEl.innerHTML = `
    <section class="page">
      ...
      <div class="toolbar">
        <label>Cerca <input type="search" id="cime-search" value="${escapeHtml(state.cimeSearch)}" placeholder="Nome cima" autocomplete="off"></label>
        <label>Gruppo ${gruppoSelect("cime-gruppo", state.gruppoFilter)}</label>
      </div>
      <div class="list">${rows || `<p class="empty">Nessuna cima.</p>`}</div>
    </section>`;
  const searchEl = document.getElementById("cime-search");
  searchEl.addEventListener("input", (e) => {
    state.cimeSearch = e.target.value;
    const caret = e.target.selectionStart;
    viewCime();
    const again = document.getElementById("cime-search");
    if (!again) return;
    again.focus();
